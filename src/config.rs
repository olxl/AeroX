...
impl ServerConfig {
    ...
    pub fn from_file<P: AsRef<Path>>(path: P) -> crate::Result<Self> {
        let content = std::fs::read_to_string(path)
            .map_err(|e| crate::AeroXError::Config(format!("读取配置文件失败: {}", e)))?;

        let config: ServerConfig = toml::from_str(&content)
            .map_err(|e| crate::AeroXError::Config(format!("解析配置文件失败: {}", e)))?;
    ...
    pub fn validate(&self) -> crate::Result<()> {
        if self.port == 0 {
            return Err(crate::AeroXError::Config("端口不能为 0".to_string()));
        }

        if self.port > 65535 {
            return Err(crate::AeroXError::Config("端口超出范围".to_string()));
        }
